...
public class GenericWorkflowExecutorTest {
    ...
    public void testPrepareMyRecordsIfNeeded() {
        ...
        assertTrue("protocol message bytes are still there", we.messageBytesCollector.getProtocolMessageBytes().length == 2);
        ...
        we.prepareMyRecordsIfNeeded(pm);
        assertTrue("protocol message bytes were used for record creation", we.messageBytesCollector.getProtocolMessageBytes().length == 0);
