            ...
            switch (key.getAlgorithm())
            {
                case "RSA":
                    ...
                    if (tlsContext.getSupportedSignatureAndHashAlgorithmsForRSA().size() == 0)
                    {
                        ...
                        {
                            //Choose one random
                            selectedSignatureHashAlgo = new SignatureAndHashAlgorithm(SignatureAlgorithm.RSA, generateRandomHashAlgorithm());
                ...
                case "EC":
                    ...
                    if (tlsContext.getSupportedSignatureAndHashAlgorithmsForEC().size() == 0)
                    {
                        ...
                        do
                        {
                            selectedSignatureHashAlgo = new SignatureAndHashAlgorithm(SignatureAlgorithm.ECDSA, generateRandomHashAlgorithm());
                            instance = Signature.getInstance(selectedSignatureHashAlgo.getJavaName());
                        }
                        while (instance == null);
