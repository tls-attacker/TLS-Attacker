...
public abstract class GenericWorkflowExecutor implements WorkflowExecutor {
    ...
    public void executeWorkflow() throws WorkflowExecutionException {
        ...
        try {
            while (workflowContext.getProtocolMessagePointer() < protocolMessages.size() && workflowContext.isProceedWorkflow()) {
     ...
     */
    protected void parseRawBytesIntoProtocolMessages(byte[] rawProtocolMessageBytes, List<ProtocolMessage> protocolMessages,
        ...
        while (dataPointer != rawProtocolMessageBytes.length && workflowContext.isProceedWorkflow()) {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("The following message was parsed: {}", pmh.getProtocolMessage()
                        .toString());
            }
    ...
    protected List<List<Record>> createListsOfRecordsOfTheSameContentType(List<Record> records) {
        ...
        while (recordPointer < records.size()) {
            ProtocolMessageType previousMessageType = ProtocolMessageType.getContentType(record
                    .getContentType().getValue());
            record = records.get(recordPointer);
            ProtocolMessageType currentMessageType = ProtocolMessageType.getContentType(record
                    .getContentType().getValue());
