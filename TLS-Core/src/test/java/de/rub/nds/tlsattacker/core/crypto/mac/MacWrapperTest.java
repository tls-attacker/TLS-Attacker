...
public class MacWrapperTest {
    ...
    public void testGOST3411() throws NoSuchAlgorithmException {
        WrappedMac mac =
            MacWrapper.getMac(ProtocolVersion.TLS10, CipherSuite.TLS_GOSTR341094_WITH_NULL_GOSTR3411, new byte[32]);
        assertEquals(MacAlgorithm.HMAC_GOSTR3411.getSize(), mac.getMacLength());

        byte[] actual = mac.calculateMac("Test data".getBytes());
        byte[] expected =
            ArrayConverter.hexStringToByteArray("9E1A18525244994DF9D9006D057B4CA01093458D40DA788A91E6324278E575DF");
        assertArrayEquals(expected, actual);

        actual = mac.calculateMac(" extended".getBytes());
        expected =
            ArrayConverter.hexStringToByteArray("C0ECFFEA5D7021D84C74227D7A7E80C2C6BD5F3C5F298E904AF9A4BA5C5E5AEF");
        assertArrayEquals(expected, actual);
    }
    ...
    public void testGOST28147IMIT() throws NoSuchAlgorithmException {
        WrappedMac mac =
            MacWrapper.getMac(ProtocolVersion.TLS12, CipherSuite.TLS_GOSTR341001_WITH_28147_CNT_IMIT, new byte[32]);
