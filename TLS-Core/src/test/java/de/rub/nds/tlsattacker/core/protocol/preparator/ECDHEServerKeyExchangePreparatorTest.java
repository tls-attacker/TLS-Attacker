...
public class ECDHEServerKeyExchangePreparatorTest extends AbstractTlsMessagePreparatorTest<
    ECDHEServerKeyExchangeMessage, ECDHEServerKeyExchangePreparator<ECDHEServerKeyExchangeMessage>> {

    public ECDHEServerKeyExchangePreparatorTest() {
        super(ECDHEServerKeyExchangeMessage::new, ECDHEServerKeyExchangeMessage::new,
            ECDHEServerKeyExchangePreparator::new);
        BadFixedRandom rnd = new BadFixedRandom((byte) 0x23);
        BadRandom random = new BadRandom(rnd, null);
        context.getConfig().setDefaultServerEcPrivateKey(new BigInteger(
    ...
    public void testPrepare() throws IOException {
        ...
        Certificate cert = Certificate.parse(new ByteArrayInputStream(ArrayConverter.hexStringToByteArray(
        ...
        context.getConfig().setDefaultExplicitCertificateKeyPair(new CertificateKeyPair(cert, new CustomECPrivateKey(
            context.getConfig().getDefaultClientEcPrivateKey(), context.getConfig().getDefaultSelectedNamedGroup())));
        context.getConfig().setAutoSelectCertificate(false);
        assertArrayEquals(ArrayConverter.concatenate(context.getClientRandom(), context.getServerRandom()),
            message.getComputations().getClientServerRandom().getValue());
        assertEquals(EllipticCurveType.NAMED_CURVE, EllipticCurveType.getCurveType(message.getGroupType().getValue()));
        ...
        assertEquals(serializedPubKeyExpected, ArrayConverter.bytesToRawHexString(message.getPublicKey().getValue()));
        assertArrayEquals(ArrayConverter.hexStringToByteArray("0601"),
        ...
        assertEquals(sigExpected, ArrayConverter.bytesToRawHexString(message.getSignature().getValue()));
    }
