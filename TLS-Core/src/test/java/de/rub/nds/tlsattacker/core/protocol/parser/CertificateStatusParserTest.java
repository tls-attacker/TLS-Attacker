/**
 * TLS-Attacker - A Modular Penetration Testing Framework for TLS
 *
 * Copyright 2014-2022 Ruhr University Bochum, Paderborn University, Hackmanit GmbH
 *
 * Licensed under Apache License, Version 2.0
 * http://www.apache.org/licenses/LICENSE-2.0.txt
 */

package de.rub.nds.tlsattacker.core.protocol.parser;

import de.rub.nds.modifiablevariable.util.ArrayConverter;
import de.rub.nds.tlsattacker.core.constants.HandshakeMessageType;
import de.rub.nds.tlsattacker.core.constants.ProtocolVersion;
import de.rub.nds.tlsattacker.core.protocol.message.CertificateStatusMessage;
import org.junit.jupiter.api.Named;
import org.junit.jupiter.params.provider.Arguments;

import java.util.List;
import java.util.stream.Stream;

public class CertificateStatusParserTest
    extends AbstractHandshakeMessageParserTest<CertificateStatusMessage, CertificateStatusParser> {

    public CertificateStatusParserTest() {
        super(CertificateStatusMessage.class, CertificateStatusParser::new,
            List.of(
                Named.of("CertificateStatusMessage::getCertificateStatusType",
                    CertificateStatusMessage::getCertificateStatusType),
                Named.of("CertificateStatusMessage::getOcspResponseLength",
                    CertificateStatusMessage::getOcspResponseLength),
                Named.of("CertificateStatusMessage::getOcspResponseBytes",
                    CertificateStatusMessage::getOcspResponseBytes)));
    }

    public static Stream<Arguments> provideTestVectors() {
        return Stream.of(Arguments.of(ProtocolVersion.TLS12, ArrayConverter.hexStringToByteArray(
            ""),
            List.of(HandshakeMessageType.CERTIFICATE_STATUS.getValue(), 531, 1, 527,
                ArrayConverter.hexStringToByteArray(
                    ""))));
    }
}
