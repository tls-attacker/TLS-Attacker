...
public class PointFormatterTest {
    ...
    public void compressionFormatTest() {
        ...
        byte[] compressed =
            PointFormatter.formatToByteArray(NamedGroup.SECP160R1, CurveFactory.getCurve(NamedGroup.SECP160R1)
                .getBasePoint(), ECPointFormat.ANSIX962_COMPRESSED_PRIME);
        assertArrayEquals(secp160r1Base, compressed);

        compressed =
            PointFormatter.formatToByteArray(NamedGroup.SECP224K1, CurveFactory.getCurve(NamedGroup.SECP224K1)
                .getBasePoint(), ECPointFormat.ANSIX962_COMPRESSED_PRIME);
        assertArrayEquals(secp224k1Base, compressed);

        compressed =
            PointFormatter.formatToByteArray(NamedGroup.SECP521R1, CurveFactory.getCurve(NamedGroup.SECP521R1)
                .getBasePoint(), ECPointFormat.ANSIX962_COMPRESSED_PRIME);
        assertArrayEquals(secp521r1Base, compressed);

        compressed =
            PointFormatter.formatToByteArray(NamedGroup.SECT193R2, CurveFactory.getCurve(NamedGroup.SECT193R2)
                .getBasePoint(), ECPointFormat.ANSIX962_COMPRESSED_CHAR2);
        assertArrayEquals(sect193r2Base, compressed);

        compressed =
            PointFormatter.formatToByteArray(NamedGroup.SECT233R1, CurveFactory.getCurve(NamedGroup.SECT233R1)
                .getBasePoint(), ECPointFormat.ANSIX962_COMPRESSED_CHAR2);
        assertArrayEquals(sect233r1Base, compressed);

        compressed =
            PointFormatter.formatToByteArray(NamedGroup.SECT571K1, CurveFactory.getCurve(NamedGroup.SECT571K1)
                .getBasePoint(), ECPointFormat.ANSIX962_COMPRESSED_CHAR2);
