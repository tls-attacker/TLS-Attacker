        ...
        LayerStack layerStack =
                new LayerStack(
                        ...
                        new ToggleableLayerWrapper<>(
                                new HttpLayer(state.getContext().getHttpContext())),
                        new ToggleableLayerWrapper<>(
                                new TcpLayer(state.getContext().getTcpContext()), false));
        state.getContext().setLayerStack(layerStack);
        ToggleableLayerWrapper wrapper1 =
                (ToggleableLayerWrapper)
                        state.getContext().getLayerStack().getLayer(HttpLayer.class);
        assert wrapper1 != null;
        ToggleableLayerWrapper wrapper2 =
                (ToggleableLayerWrapper)
                        state.getContext().getLayerStack().getLayer(TcpLayer.class);
        assert wrapper2 != null;

        ToggleLayerAction action =
                new ToggleLayerAction(ImplementedLayers.HTTP, ImplementedLayers.TCP);
