...
public class Dtls12WorkflowExecutorTest {
    ...
    public void testParseServerHello() throws SocketException, UnknownHostException, IOException {
        ...
        config.setTransportHandlerType(TransportHandlerType.UDP);
        UDPTransportHandler th = (UDPTransportHandler) new ConfigHandler().initializeTransportHandler(config);
        try (DatagramSocket sender = new DatagramSocket(4445, InetAddress.getByName("127.0.0.1"))) {
            sender.connect(th.getLocalAddress(), th.getLocalPort());
    ...
    // config.setMyConnectionEnd(ConnectionEnd.CLIENT);
    // TlsContext context = new TlsContext(config);
    // Dtls12WorkflowExecutor workflowExecutor = new Dtls12WorkflowExecutor(th,
    // context);
    // workflowExecutor.executeWorkflow();
