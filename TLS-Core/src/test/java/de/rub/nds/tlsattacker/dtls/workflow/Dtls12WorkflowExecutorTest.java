...
public class Dtls12WorkflowExecutorTest {
    ...
    // }
    //
    // public void testParseHelloVerifyRequest() throws SocketException,
    ...
    // trace.add(new ReceiveAction(new HelloVerifyRequestMessage(config)));
    // context.setWorkflowTrace(trace);
    // DtlsWorkflowExecutor workflowExecutor = new
    // DtlsWorkflowExecutor(context);
    // TransportHandler th = context.getTransportHandler();
    // try (DatagramSocket sender = new DatagramSocket(4444,
    // InetAddress.getByName("127.0.0.1"))) {
    // sender.connect(Inet4Address.getByName("127.0.0.1"), th.getPort());
    //
    ...
    // assertFalse(trace.getActuallyRecievedHandshakeMessagesOfType(HandshakeMessageType.HELLO_VERIFY_REQUEST)
    // .isEmpty());
    // }
    //
    // /**
    // * Tests the parsing of 3 real DTLS server hello fragments
    // *
    ...
    // */
    //
    // public void testParseServerHello() throws SocketException,
    // UnknownHostException, IOException {
    // TlsConfig config = new TlsConfig();
    // config.setEnforceSettings(true);
    // config.setHighestProtocolVersion(ProtocolVersion.DTLS12);
    // config.setHost("127.0.0.1:4445");
    // config.setTransportHandlerType(TransportHandlerType.UDP);
    // UDPTransportHandler th = (UDPTransportHandler) new
    // ConfigHandler().initializeTransportHandler(config);
    // try (DatagramSocket sender = new DatagramSocket(4445,
    // InetAddress.getByName("127.0.0.1"))) {
    ...
    // context.setWorkflowTrace(trace);
    // DtlsWorkflowExecutor workflowExecutor = new DtlsWorkflowExecutor(th,
    // context);
    // workflowExecutor.getActionExecutor().getHandshakeFragmentHandler().incrementExpectedHandshakeMessageSeq();
    // workflowExecutor.executeWorkflow();
    // assertFalse(trace.getActuallyRecievedHandshakeMessagesOfType(HandshakeMessageType.SERVER_HELLO).isEmpty());
    // assertFalse(trace.getActuallyRecievedHandshakeMessagesOfType(HandshakeMessageType.CERTIFICATE).isEmpty());
    // assertFalse(trace.getActuallyRecievedHandshakeMessagesOfType(HandshakeMessageType.SERVER_HELLO_DONE).isEmpty());
    // }
    //
    ...
    // assertFalse(trace.getActuallyRecievedHandshakeMessagesOfType(HandshakeMessageType.FINISHED).isEmpty());
    // }
