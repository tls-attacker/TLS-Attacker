...
public class CertificateStatusRequestV2ExtensionParserTest {
    ...
    private final List<RequestItemV2> list = Arrays.asList(new RequestItemV2(1, 21, 0xb, Arrays.asList(new ResponderId(3, new byte[]{0x01, 0x02, 0x03}), new ResponderId(4, new byte[]{0x04, 0x05, 0x06, 0x07})), 6, hexStringToByteArray("010203040506"), hexStringToByteArray("010015000B00030102030004040506070006010203040506")), new RequestItemV2(1, 21, 0xb, Arrays.asList(new ResponderId(3, new byte[]{0x01, 0x02, 0x03}), new ResponderId(4, new byte[]{0x04, 0x05, 0x06, 0x07})), 6, hexStringToByteArray("010203040506"), hexStringToByteArray("010015000B00030102030004040506070006010203040506")));
    ...
    public void testParser() {
        CertificateStatusRequestV2ExtensionParser parser = new CertificateStatusRequestV2ExtensionParser(startPosition, parseBytes);
    ...
    public static void assertRequestItemV2List(List<RequestItemV2> listExpected, List<RequestItemV2> listActual) {
        ...
        for (int i = 0; i < listExpected.size(); i++) {
            ...
            assertArrayEquals(itemExpected.getRequestExtensions().getValue(), itemActual.getRequestExtensions().getValue());
            assertEquals(itemExpected.getRequestExtensionsLength().getValue(), itemActual.getRequestExtensionsLength().getValue());
            ...
            assertArrayEquals(itemExpected.getResponderIdListBytes().getValue(), itemActual.getResponderIdListBytes().getValue());
            assertEquals(itemExpected.getResponderIdListLength().getValue(), itemActual.getResponderIdListLength().getValue());
        }
    }
