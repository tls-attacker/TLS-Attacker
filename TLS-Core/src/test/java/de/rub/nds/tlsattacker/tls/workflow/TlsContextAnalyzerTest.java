...
import de.rub.nds.tlsattacker.unittest.WorkFlowTraceFakeExecuter;
...
public class TlsContextAnalyzerTest {
    ...
    //
    // ProtocolMessage pm =
    // context.getWorkflowTrace().getFirstConfiguredProtocolMessage(ProtocolMessageType.CHANGE_CIPHER_SPEC);
    ...
    // context.getWorkflowTrace().getConfiguredHandshakeMessagePositions(HandshakeMessageType.FINISHED)
    // .get(0);
    // context.getWorkflowTrace().add(new ReceiveAction(new AlertMessage()));
    ...
    //
    // int position =
    // context.getWorkflowTrace().getConfiguredProtocolMessagePositions(ProtocolMessageType.CHANGE_CIPHER_SPEC)
    // .get(0);
