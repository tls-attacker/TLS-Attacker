...
public class CertificateVerifyHandlerTest {
    ...
    // 0x00, 0x00, 0x09 };
    // byte[] sigHashAlg = { HashAlgorithm.SHA512.getValue(),
    // SignatureAlgorithm.RSA.getValue() };
    // inputBytes = ArrayConverter.concatenate(inputBytes, sigHashAlg, new
    // byte[] { 0x00, 0x05 }, new byte[] { 0x25,
    // 0x26, 0x27, 0x28, 0x29 });
    ...
    // message.getSignatureHashAlgorithm()
    // .getValue());
    // assertTrue("Confirm Signature Length",
    // message.getSignatureLength().getValue() == 5);
    // assertTrue("Confirm Signature",
    // Arrays.equals(message.getSignature().getValue(), new byte[] { 0x25, 0x26,
    // 0x27, 0x28, 0x29 }));
    //
    // }
