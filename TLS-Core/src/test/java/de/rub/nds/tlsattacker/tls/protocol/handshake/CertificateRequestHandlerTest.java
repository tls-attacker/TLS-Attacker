...
public class CertificateRequestHandlerTest {
    ...
    // handler.setProtocolMessage(new CertificateRequestMessage(new
    // TlsConfig()));
    //
    ...
    // new byte[] { HandshakeMessageType.CERTIFICATE_REQUEST.getValue() }, new
    // byte[] { 0x00, 0x00, 0x12 },
    // new byte[] { 0x01 }, message.getClientCertificateTypes().getValue(), new
    // byte[] { 0x00, 0x0C }, message
    ...
    //
    // byte[] inputBytes = {
    // HandshakeMessageType.CERTIFICATE_REQUEST.getValue(), 0x00, 0x00, 0x07,
    // 0x01,
    // ClientCertificateType.RSA_SIGN.getValue(), 0x00, 0x02 };
    // byte[] sigHashAlg = new SignatureAndHashAlgorithm(SignatureAlgorithm.RSA,
    // HashAlgorithm.SHA512).getByteValue();
    // inputBytes = ArrayConverter.concatenate(inputBytes, sigHashAlg, new
    // byte[] { 0x00, 0x00 });
    ...
    // ClientCertificateType.RSA_SIGN.getValue(), message
    ...
    // assertArrayEquals("Confirm SignatureAndHashAlgorithm type", sigHashAlg,
