...
public class ReceiveMessageHelper {
	...
	private void processRecordGroup(RecordGroup recordGroup, TlsContext context, List<ProtocolMessage> messageFragments,
	...
	private boolean shouldContinue(List<ProtocolMessage> expectedMessages, List<ProtocolMessage> receivedMessages,
		...
		boolean receivedAllExpectedMessages = receivedAllExpectedMessage(expectedMessages, receivedMessages,
	...
	private List<ProtocolMessage> handleCleanBytes(byte[] cleanProtocolMessageBytes, ProtocolMessageType typeFromRecord,
			TlsContext context, boolean handleHandshakeAsDtlsFragments) {
		int dataPointer = 0;
	...
	/*
	 * Processes a list of arbitrary-ordered fragments. The idea is: 1. we assemble
	 * fragments into "fragmented messages" 2. we extract the messages from
	 * fragments but only update the context for fragments whose message sequence is
	 * next for processing.
	 */
	private List<ProtocolMessage> processDtlsFragments(List<DtlsHandshakeMessageFragment> fragments,
			TlsContext context) {
		...
		// message was formed
		DtlsHandshakeMessageFragment fragmentedMessage = manager
				.getFragmentedMessage(context.getDtlsNextReceiveSequenceNumber());
