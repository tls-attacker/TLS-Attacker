...
public class ExtensionListParser extends Parser<List<ExtensionMessage>> {
    ...
    public void parse(List<ExtensionMessage> extensionList) {
        while (getBytesLeft() > 0) {
            ...
            } else if(parser instanceof CertificateStatusRequestExtensionParser) {
                ((CertificateStatusRequestExtensionParser) parser).setHelloRetryRequestHint(helloRetryRequestHint);
            }
