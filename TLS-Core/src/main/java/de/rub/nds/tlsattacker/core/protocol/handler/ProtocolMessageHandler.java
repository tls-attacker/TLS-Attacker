...
/**
 * @param <Message> The ProtocolMessage that should be handled
 */
public abstract class ProtocolMessageHandler<Message extends ProtocolMessage> extends Handler<Message> {
    ...
    /**
     * @param tlsContext The Context which should be Adjusted with this Handler
     */
     ...
     *
     * @param message The message that should be prepared
     * @param withPrepare if the prepare function should be called or only the
     * rest
     * @return message in bytes
     */
    ...
    public ParserResult parseMessage(byte[] message, int pointer, boolean onlyParse) {
        ...
        if (tlsContext.getChooser().getSelectedProtocolVersion().isDTLS() && parsedMessage instanceof HandshakeMessage
            ...
            System.out.println("Setting correct message sequence:" + tlsContext.getDtlsReadHandshakeMessageSequence());
     ...
     * ProtocolMessage
     *
     * @param message The Message for which this context should be adjusted
     */
