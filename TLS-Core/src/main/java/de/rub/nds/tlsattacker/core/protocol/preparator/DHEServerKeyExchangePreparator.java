...
public class DHEServerKeyExchangePreparator<T extends DHEServerKeyExchangeMessage> extends
        ...
        byte[] dhParams =
            ArrayConverter
                .concatenate(ArrayConverter.intToBytes(msg.getModulusLength().getValue(),
                    HandshakeByteLength.DH_MODULUS_LENGTH), msg.getModulus().getValue(), ArrayConverter.intToBytes(msg
                    .getGeneratorLength().getValue(), HandshakeByteLength.DH_GENERATOR_LENGTH), msg.getGenerator()
                    .getValue(), ArrayConverter.intToBytes(msg.getPublicKeyLength().getValue(),
    ...
    protected void preparePublicKey(T msg) {
        ...
        try {
            ...
            publicKey =
                generator.modPow(msg.getComputations().getPrivateKey().getValue(), msg.getComputations().getModulus()
                    .getValue());
        } catch (Exception e) {
