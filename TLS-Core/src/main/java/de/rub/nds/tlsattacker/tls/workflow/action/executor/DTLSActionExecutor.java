...
public class DTLSActionExecutor extends ActionExecutor {
    ...
    // LogManager.getLogger(DTLSActionExecutor.class);
    //
    ...
    // throw new
    // WorkflowExecutionException("Retransmit Counter reached Max Retransmits!");
    // }
    //
    ...
    //
    // @Override
    // public List<ProtocolMessage> receiveMessages(List<ProtocolMessage>
    // messages) {
    ...
    // }
    // } while (continueReceiving(receivedMessage != null, messages,
    // receivedMessages));
    // return receivedMessages;
    // }
    //
    // private boolean continueReceiving(boolean receivedMessage,
    ...
    // }
    //
    // private boolean receivedExpected(List<ProtocolMessage> expectedMessages,
    // List<ProtocolMessage> received) {
    // int min = 0;
    ...
    // }
    //
    // private void handleMyProtocolMessage(ProtocolMessage pm) throws
    // IOException {
    // LOGGER.debug("Preparing the following protocol message to send: {}",
    // pm.getClass());
    // if (pm.getProtocolMessageType() == ProtocolMessageType.HANDSHAKE) {
    // handleMyHandshakeMessage((HandshakeMessage) pm);
    // } else if (pm.getProtocolMessageType() ==
    ...
    // * @throws IOException
    // */
    // private void handleMyNonHandshakeMessage(ProtocolMessage protocolMessage)
    // throws IOException {
    // ProtocolMessageHandler pmh = null;//
    // TODO//protocolMessage.getProtocolMessageHandler(tlsContext);
    //
    // byte[] messageBytes = pmh.prepareMessage(protocolMessage);
    // if (protocolMessage.getRecords() == null ||
    // protocolMessage.getRecords().isEmpty()) {
    // protocolMessage.addRecord(new Record());
    // }
    // byte[] record = recordHandler.prepareRecords(messageBytes,
    // protocolMessage.getProtocolMessageType(),
    ...
    // * @throws IOException
    // */
    // private void handleMyChangeCipherSpecMessage(ProtocolMessage
    // protocolMessage) throws IOException {
    // ProtocolMessageHandler pmh = null; //
    // TODO//protocolMessage.getProtocolMessageHandler(tlsContext);
    // byte[] messageBytes = pmh.prepareMessage(protocolMessage);
    //
    ...
    // }
    //
    // private void handleMyHandshakeMessage(HandshakeMessage handshakeMessage)
    // throws IOException {
    // HandshakeMessageHandler pmh = (HandshakeMessageHandler)
    ...
    // ArrayConverter.concatenate(handshakeMessageSendBuffer,
    // handshakeFragmentHandler.fragmentHandshakeMessage(handshakeMessageBytes,
    // maxPacketSize - 25));
    //
    ...
    //
    // bufferSendData(recordHandler.prepareRecords(handshakeMessageSendBuffer,
    ...
    // currentRecordSize = (recordSendBuffer[pointer + 11] << 8) +
    // (recordSendBuffer[pointer + 12] & 0xFF) + 13;
    ...
    // Arrays.copyOfRange(recordSendBuffer, pointer, pointer +
    // currentRecordSize));
    // recordSendBuffer = Arrays.copyOfRange(recordSendBuffer, pointer +
    // currentRecordSize,
    ...
    // long endTimeMillies = System.currentTimeMillis() +
    // maxWaitForExpectedRecord;
    ...
    // continue;
    // }
    // rcvRecordContentType =
    // ProtocolMessageType.getContentType(rcvRecord.getContentType().getValue());
    ...
    // ProtocolMessageType rcvRecordContentType =
    // ProtocolMessageType.getContentType(rcvRecord.getContentType()
    // .getValue());
    // ProtocolMessageHandler pmh = HandlerFactory.getHandler(tlsContext,
    // rcvRecordContentType,
    // HandshakeMessageType.getMessageType(rawMessageBytes[messageParseBufferOffset]));
    // ParserResult result = pmh.parseMessage(rawMessageBytes,
    // messageParseBufferOffset);
    // ProtocolMessage protocolMessage = result.getMessage();
    // messageParseBufferOffset = result.getParserPosition();
    //
    // LOGGER.debug("The following message was parsed: {}",
    // protocolMessage.toString());
    //
    // switch (protocolMessage.getProtocolMessageType()) {
    // case ALERT:
    // if (isIncomingAlertFatal(protocolMessage) &&
    // !tlsContext.getConfig().isFuzzingMode()) {
    // throw new
    // WorkflowExecutionException("Received a fatal Alert, aborting!");
    // }
    // break;
    // case HANDSHAKE:
    ...
    // return AlertLevel.getAlertLevel(am.getLevel().getValue()) !=
    // AlertLevel.FATAL;
    // }
    //
    ...
    // }
    //
    // private void updateFlight(ProtocolMessage protocolMessage,
    // ProtocolMessage previousMessage) {
