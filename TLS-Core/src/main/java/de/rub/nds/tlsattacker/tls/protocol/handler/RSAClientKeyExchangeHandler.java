...
public class RSAClientKeyExchangeHandler extends ClientKeyExchangeHandler<RSAClientKeyExchangeMessage> {
    ...
    // throw new
    // WorkflowExecutionException("Could not parse server certificate", ex);
    // }
    ...
    // } else {
    // throw new
    // WorkflowExecutionException("Cannot use non-RSA public Key in RSA-ClientKeyExchangeHandler");
    // }
    // } else {
    ...
    // int randomByteLength = keyByteLength -
    // HandshakeByteLength.PREMASTER_SECRET - 3;
    ...
    //
    // protocolMessage.setPlainPaddedPremasterSecret(ArrayConverter.concatenate(new
    // byte[] { 0x00, 0x02 }, padding,
    // new byte[] { 0x00 }, protocolMessage.getPremasterSecret().getValue()));
    //
    // byte[] paddedPremasterSecret =
    // protocolMessage.getPlainPaddedPremasterSecret().getValue();
    //
    ...
    // }
    // if (new
    // BigInteger(paddedPremasterSecret).compareTo(publicKey.getModulus()) == 1)
    // {
    // if (tlsContext.getConfig().isFuzzingMode()) {
    // paddedPremasterSecret = masterSecret;
    // } else {
    // throw new
    // IllegalStateException("Trying to encrypt more data then modulus size!");
    // }
    // }
    // byte[] encrypted = null;
    // try {
    ...
    // protocolMessage.setEncryptedPremasterSecret(Arrays.copyOfRange(message,
    // currentPointer, nextPointer));
    //
    // byte[] encryptedPremasterSecret =
    // protocolMessage.getEncryptedPremasterSecret().getValue();
    //
    // KeyStore ks = tlsContext.getConfig().getKeyStore();
    //
    // try {
    // Key key = ks.getKey(tlsContext.getConfig().getAlias(),
    // tlsContext.getConfig().getPassword().toCharArray());
    ...
    // "Something went wrong loading key from Keystore or decrypting Premastersecret",
    // ex);
    // }
    //
    // byte[] plainPaddedPremasterSecret =
    // protocolMessage.getPlainPaddedPremasterSecret().getValue();
    //
    // int plainPaddedPremasterSecretLength = plainPaddedPremasterSecret.length;
    //
    // int plainPaddedPremasterSecretoffset = plainPaddedPremasterSecretLength -
    // 48;
    //
    // byte[] premasterSecret = Arrays.copyOfRange(plainPaddedPremasterSecret,
    // plainPaddedPremasterSecretoffset,
    ...
    @Override
    protected Preparator getPreparator(RSAClientKeyExchangeMessage message) {
