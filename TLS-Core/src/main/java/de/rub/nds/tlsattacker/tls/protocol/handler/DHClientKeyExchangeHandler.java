...
public class DHClientKeyExchangeHandler extends ClientKeyExchangeHandler<DHClientKeyExchangeMessage> {
    ...
    // protocolMessage.setSerializedPublicKey(Arrays.copyOfRange(message,
    // currentPointer, nextPointer));
    // BigInteger publicKey = new BigInteger(1, Arrays.copyOfRange(message,
    // currentPointer, nextPointer));
    // protocolMessage.setY(publicKey);
    //
    // byte[] premasterSecret;
    //
    // DHPublicKeyParameters clientPubParameters = new
    // DHPublicKeyParameters(protocolMessage.getY().getValue(),
    // tlsContext.getServerDHParameters().getPublicKey().getParameters());
    //
    // premasterSecret =
    // TlsDHUtils.calculateDHBasicAgreement(clientPubParameters,
    ...
    // AlgorithmResolver.getPRFAlgorithm(tlsContext.getConfig()
    // .getHighestProtocolVersion(), tlsContext.getSelectedCipherSuite());
    ...
    // if
    // (!keyInfo.getAlgorithm().getAlgorithm().equals(X9ObjectIdentifiers.dhpublicnumber))
    // {
    ...
    // tlsContext.getServerDHParameters()
    ...
    // try {
    // kp = TlsDHUtils.generateDHKeyPair(new SecureRandom(),
    // tlsContext.getServerDHParameters().getPublicKey()
    // .getParameters());
    //
    ...
    // DHPrivateKeyParameters dhPrivate = (DHPrivateKeyParameters)
    // kp.getPrivate();
    //
    ...
    // DHParameters newParams = new
    // DHParameters(protocolMessage.getP().getValue(),
    ...
    // DHPrivateKeyParameters newDhPrivate = new
    // DHPrivateKeyParameters(protocolMessage.getX().getValue(), newParams);
    // try {
    // premasterSecret =
    // TlsDHUtils.calculateDHBasicAgreement(tlsContext.getServerDHParameters().getPublicKey(),
    // newDhPrivate);
    // } catch (IllegalArgumentException e) {
    // if (tlsContext.getConfig().isFuzzingMode()) {
    ...
    // BigIntegers.asUnsignedByteArray(protocolMessage.getY().getValue());
    ...
    // .getSerializedPublicKeyLength().getValue(),
    // HandshakeByteLength.DH_PARAM_LENGTH), protocolMessage
    ...
    @Override
    protected Preparator getPreparator(DHClientKeyExchangeMessage message) {
        throw new UnsupportedOperationException("Not supported yet."); // To
                                                                       // change
                                                                       // body
                                                                       // of
