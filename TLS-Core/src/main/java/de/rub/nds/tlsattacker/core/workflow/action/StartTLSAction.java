...
public class StartTLSAction extends ConnectionBoundAction {
     ...
     * LayerStack does not fit. For now, only SMTP and POP3 are supported. Users are still responsible for
    ...
    public void execute(State state) throws ActionExecutionException {
        ...
        if (!EnumSet.of(ImplementedLayers.SMTP, ImplementedLayers.POP3).contains(topLevelType)) {
            throw new ActionExecutionException("STARTTLS is not defined for the top-level protocol " + topLevelType);
        }
        if (isExecuted()) {
