...
public class VRFYReplyParser extends SmtpReplyParser<SmtpVRFYReply> {
    ...
    private final String[] validStatusCodes = new String[]{
            "250", "251", "252",
            "502", "504", "550", "551", "553" // technically, 502 isn't rfc5321-compliant, but it's recommended by the rfc nonetheless
    };
    ...
    public void parse(SmtpVRFYReply reply) {
        ...
        if (lines.isEmpty())
            throw new ParserException("Malformed VRFY-Reply: Reply is empty.");
    ...
    private void parseResponse(String statusCode, String line, SmtpVRFYReply reply) {
        if (isNormalResponse(statusCode)) {
            ...
            if (!couldBeParsed)
                throw new ParserException("Malformed VRFY-Reply: Mailbox is invalid or reply may be malformed.");
    ...
    /**
     * Note: According to RFC5321, servers may either note the ambiguity or provide the ambiguous mailboxes to the user.
     * Since there is no fixed description that is to be used to note the ambiguity, any string that doesn't contain
     ...
     */
    private boolean parseNormalResponse(String line, SmtpVRFYReply reply, boolean mayContainFullName) {
    ...
    private void trySettingDescription(String description, SmtpVRFYReply reply) {
        if (reply.getDescription() != null) // TODO: re-check RFC5321 for possible exceptions to this.
            throw new ParserException("Malformed VRFY-Reply: Reply may not contain multiple descriptions.");
    ...
    private int findMailboxStartIndex(String line) {
        if (line.length() < 5) // 5 because it needs to contain at least something of the form: <a@a>
            throw new ParserException("Malformed VRFY-Reply: Mailbox is too short.");

        if (line.charAt(line.length()-1) != '>')
            throw new ParserException("Malformed VRFY-Reply: Mailbox is not enclosed in pointed brackets <>.");
    ...
    private String parseStatusCode(String line) {
        ...
        if (statusCode == null)
            throw new ParserException("Malformed VRFY-Reply: String starts with invalid status code or delimiter.");
