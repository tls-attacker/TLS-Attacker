...
public class MessageFragmentCollector {
    ...
    public MessageFragmentCollector() {
        fragmentData = new TreeSet<>(
        		new Comparator<DtlsHandshakeMessageFragment>() {
					@Override
    ...
    private void parseMessageSeq(DtlsHandshakeMessageFragment fragment) {
    	...
    	if (messageSeq == null) {
        	...
        	if (!messageSeq.equals(fMessageSeq)) {
        		LOGGER.warn("Found an unffiting fragment! Message seq before:" + messageSeq + " inserted fragment message seq:"
                        + fMessageSeq);
        	}
        }
    }
    ...
    private void parseLength(DtlsHandshakeMessageFragment fragment) {
    	...
    	if (length == null) {
        	...
        	if (!length.equals(fLength)) {
        		LOGGER.warn("Found an unffiting fragment! Message length before:" + length + " inserted fragment length:"
                        + fLength);
        	}
        }
    }
    ...
    /*
     * TODO: take into account offset when building the combined content (then we may no longer need TreeSets)
     * also, there are no inconsistency checks
     */
    ...
    public byte[] getFragmentedMessageAsByteArray() {
    	...
    	DtlsHandshakeMessageFragmentSerializer serializer = new DtlsHandshakeMessageFragmentSerializer(combinedFragment, null);
