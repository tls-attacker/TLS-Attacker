...
/**
 * Parses Pop3 replies from InputStream. The default implementation only parses the status code
 *  and the human readable message. If more complex parsing is needed, the parse method can be
...
public abstract class Pop3ReplyParser<ReplyT extends Pop3Reply> extends Pop3MessageParser<ReplyT> {
    ...
    public List<String> readWholeReply() {
        ...
        if (!lines.get(lines.size() - 1).matches("^\\.") ) {
            throw new ParserException("No termination octet has been sent: " + lines.get(lines.size() - 1));
        }
