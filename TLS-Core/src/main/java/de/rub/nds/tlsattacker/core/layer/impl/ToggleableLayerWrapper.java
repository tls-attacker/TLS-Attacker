 ...
 * <p>If the layer is inactive, it will not send nor receive any data specified by its own {@link LayerConfiguration}.
 * <p>Data supplied via {@link #sendData(LayerProcessingHint, byte[])} will be passed through the layer unchanged.
 * <p>Methods inquiring the layer's state will return empty (but not null) results when inactive.
 * <p>The intended use case is to allow for easy toggling of layers in the LayerStack via the {Enable, Disable, Toggle}LayerAction.
 * <p><b>Important:</b>The ToggleableLayerWrapper is intended to be used for optional intermediate layers and assumes a lower layer. While this assumption is not enforced, it is not supported to use a toggleable layer as the lowest layer in the stack.
 * @param <H>
 * @param <C>
 * @see de.rub.nds.tlsattacker.core.workflow.action.EnableLayerAction
 * @see de.rub.nds.tlsattacker.core.workflow.action.DisableLayerAction
 * @see de.rub.nds.tlsattacker.core.workflow.action.ToggleLayerAction
 */
public class ToggleableLayerWrapper<H extends LayerProcessingHint, C extends DataContainer> extends ProtocolLayer<H, C> {
