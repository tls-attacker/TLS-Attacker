...
public enum SignedCertificateTimestampEntryType {
    ...
    public static SignedCertificateTimestampEntryType decodeVersion(byte[] encodedVersion)
        ...
        switch (encoded) {
            ...
            default:
                throw new ParserException(
                        "SignedCertificateTimestampEntryType with byte value \""
                                + encoded
                                + "\" is not supported.");
        }
    }

    public static byte[] encodeVersion(SignedCertificateTimestampEntryType entryType)
            throws ParserException {
        switch (entryType) {
            case X509ChainEntry:
