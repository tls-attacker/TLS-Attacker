    ...
    private void testSupportedSignatureAndHashAlgorithms(ProtocolVersion pv) {
        for (SignatureAndHashAlgorithm algorithm : SignatureAndHashAlgorithm.values()) {
            ...
            if (success) {
                ...
                if (lastTlsContext.getWorkflowTrace().getActuallyRecievedHandshakeMessagesOfType(HandshakeMessageType.SERVER_KEY_EXCHANGE).get(0) != null) {
                    ServerKeyExchangeMessage skm = (ServerKeyExchangeMessage) lastTlsContext.getWorkflowTrace().getActuallyRecievedHandshakeMessagesOfType(HandshakeMessageType.SERVER_KEY_EXCHANGE).get(0);
