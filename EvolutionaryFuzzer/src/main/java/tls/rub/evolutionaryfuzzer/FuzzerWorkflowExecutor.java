...
/**
 * A Custom WorkflowExecutor, which executes WorkflowTraces without the need to
 * know with which messages the Server answers in advance //TODO IMPLEMENT
 * @deprecated 
 * @author Robert Merget - robert.merget@rub.de
 */
public class FuzzerWorkflowExecutor implements WorkflowExecutor
{

    private static final Logger LOGGER = LogManager.getLogger(FuzzerWorkflowExecutor.class);
     ...
     * @param tlsContext
     */
    public FuzzerWorkflowExecutor(TransportHandler transportHandler, TlsContext tlsContext)
    {
    ...
    public void executeWorkflow() throws WorkflowExecutionException
    {
        ...
        ensureMyLastProtocolMessagesHaveRecords(protocolMessages); //brauch ich das?
        try
        {
            ...
            { //?
                ...
                { //das sendet einfach meine nachrichten
                    handleMyProtocolMessage(protocolMessages);
                }
                else
                {
                    handleProtocolMessagesFromPeer(protocolMessages); //das empfängt nachrichten
                }
            }
        }
    ...
    protected void ensureMyLastProtocolMessagesHaveRecords(List<ProtocolMessage> protocolMessages)
    {
        for (int pmPointer = 0; pmPointer < protocolMessages.size(); pmPointer++)
        {
            ...
            if (handlingMyLastProtocolMessageWithContentType(protocolMessages, pmPointer))
            {
                if(pm ==null)
                {
                    System.out.println("Null in protocolmessages");
                }
                if (pm.getRecords() == null || pm.getRecords().isEmpty())
                {
    ...
    private static final java.util.logging.Logger LOG = java.util.logging.Logger.getLogger(FuzzerWorkflowExecutor.class.getName());
}
