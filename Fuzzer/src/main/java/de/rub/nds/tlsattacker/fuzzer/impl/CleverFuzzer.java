 ...
 */
public class CleverFuzzer extends Fuzzer {

    public static Logger LOGGER = LogManager.getLogger(CleverFuzzer.class);

    private final CleverFuzzerConfig fuzzerConfig;
...
//                LOGGER.info(sce.getServerErrorOutputString());
    ...
    private void startFuzzing(String logFolder) throws IOException,
            ConfigurationException, JAXBException, IllegalAccessException, IllegalArgumentException {
        LOGGER.log(LogLevel.CONSOLE_OUTPUT, "Starting fuzzing {}", fuzzingName);
        phase1(logFolder);
        LOGGER.log(LogLevel.CONSOLE_OUTPUT, "The following variables do not influence handshake (check manually for false-positives): {} ", variablesWithoutHandshakeInfluence);
    ...
    private void phase1(String logFolder) throws IOException, JAXBException {
        for (WorkflowTrace trace : validWorkflowTraces) {
            List<ModifiableVariableField> fields = FuzzingHelper
                    .getAllModifiableVariableFieldsRecursively(trace, ConnectionEnd.CLIENT);
...
//                executeProtocolModification(tmpWorkflow, tmpTlsContext.getMyConnectionEnd());
    ...
    private void executeProtocolModificationPhase(WorkflowTrace workflow, ConnectionEnd myConnectionEnd) {
        ...
        i = 0;
        while (i < MAX_MODIFICATION_COUNT && FuzzingHelper.executeFuzzingUnit(fuzzerConfig.getNotSendingMessagePercantage())) {
            i++;
     ...
     * @throws JAXBException
     */
    private void analyzeServerTerminationAndWriteFile(ServerStartCommandExecutor sce, String folder, String variableName,
            String workflowName, long phase, WorkflowTrace workflow) throws IOException, JAXBException {
        if (fuzzerConfig.containsServerCommand() && sce.isServerTerminated()) {
            interruptFuzzing = true;
            FileOutputStream fos = new FileOutputStream(folder + "/terminated" + variableName + workflowName + Long.toString(phase) + ".xml");
