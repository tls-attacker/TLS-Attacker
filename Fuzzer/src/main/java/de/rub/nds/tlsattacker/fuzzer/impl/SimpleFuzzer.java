...
public class SimpleFuzzer extends Fuzzer {
    ...
    private static final int MAX_SYSTEMATIC_MODIFICATIONS = 5;
    ...
    private void startFuzzing(ConfigHandler configHandler, Certificate certificate, ServerStartCommandExecutor sce,
            String folder) throws IOException, ConfigurationException, JAXBException, IllegalAccessException, IllegalArgumentException {
    ...
    private void startRandomFuzzing(ConfigHandler configHandler, Certificate certificate,
        ...
        while (!interruptFuzzing) {
            ...
            // them to a file
            analyzeResultingTlsContextAndWriteFile(tlsContext, folder, step);
        }
    }
    ...
    private void startSystematicFuzzing(ConfigHandler configHandler, Certificate certificate,
        ...
        while (!interruptFuzzing) {
            ...
            for (int fieldNumber = 0; fieldNumber < fields.size(); fieldNumber++) {
                for (int i = 0; i < MAX_SYSTEMATIC_MODIFICATIONS; i++) {
                    TlsContext tlsContext = configHandler.initializeTlsContext(fuzzerConfig);
                    WorkflowTrace workflow = tlsContext.getWorkflowTrace();
                    ...
                    FuzzingHelper.executeModifiableVariableModification((ModifiableVariableHolder) mvField.getObject(), mvField.getField());
     ...
     * @throws IOException
     */
    private void analyzeResultingTlsContextAndWriteFile(TlsContext tlsContext, String folder, long step) throws JAXBException,
            IOException {
