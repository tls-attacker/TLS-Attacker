...
public class InvalidCurveAttacker extends Attacker<InvalidCurveAttackConfig> {
    ...
    private EllipticCurveOverFp buildTwistedCurve() {
        ...
        BigInteger modA =
            intendedCurve.getFieldA().getData().multiply(config.getCurveTwistD().pow(2))
                .mod(intendedCurve.getModulus());
        BigInteger modB =
            intendedCurve.getFieldB().getData().multiply(config.getCurveTwistD().pow(3))
                .mod(intendedCurve.getModulus());
        ...
        for (TlsTask tlsTask : taskList) {
            ...
            if (!task.isHasError()) {
                ...
                if (!(WorkflowTraceUtil.getLastReceivedMessage(trace) != null
                    && WorkflowTraceUtil.getLastReceivedMessage(trace) instanceof HandshakeMessage && ((HandshakeMessage) WorkflowTraceUtil
